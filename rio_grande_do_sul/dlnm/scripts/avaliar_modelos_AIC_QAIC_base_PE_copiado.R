...
base_dir <- "c:/Users/DaniLinda/Desktop/Doutorado PE/analises_individuais/RIO GRANDE DO SUL DLNM MASS + OFFSET"
...
cat("[OFFSET] Arquivos encontrados:", length(files), "\n")
res <- list()

for (f in files) {
  cat("[OFFSET] Processando:", f, "\n")
  ...
  if (is.na(modname)) {
    cat("[OFFSET] Nenhum objeto de modelo encontrado em:", f, "\n")
    next
  }

  m <- get(modname, envir = e)
  modelo_file <- sub("^resultados_(.*)_offset\\.RData$", "\\1", basename(f))
  tipo <- if (grepl("_precip_humid$", modelo_file)) "combinado_offset" else "individual_offset"
...
if (length(res) == 0) {
  cat("[OFFSET] Nenhum modelo encontrado. Verifique os arquivos .RData com sufixo _offset.\n")
...
cat("[OFFSET] Ranking AIC gerado em:", file.path(out_dir, "ranking_modelos_offset.csv"), "\n")
cat("[OFFSET] Ranking BIC gerado em:", file.path(out_dir, "ranking_modelos_por_BIC_offset.csv"), "\n")
cat("[OFFSET] Relatorio gerado em:", file.path(out_dir, "RELATORIO_RANKING_MODELOS_OFFSET.md"), "\n")
