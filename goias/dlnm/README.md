...
Nova versao dos codigos de GOIAS com:
...
- `scripts/`: funcoes e scripts de execucao.
- `resultados/figuras/`: figuras DLNM com sufixo `_offset`.
- `resultados/summaries/`: sumarios de modelo, QC, n_final, multicolinearidade.
...
## Scripts principais
- `scripts/functions_dlnm_offset_goias.R` (funcoes obrigatorias e pipeline)
...
## Comando principal
Executar em `GO DLNM MASS + OFFSET/scripts`:

```bash
Rscript run_all_models_offset.R
```

Se necessario apenas consolidar relatorios:
