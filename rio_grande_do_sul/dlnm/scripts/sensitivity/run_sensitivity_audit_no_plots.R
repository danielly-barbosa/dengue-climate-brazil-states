...
audit_dir <- file.path(sensitivity_root, "audit_no_plots")
audit_meta_dir <- file.path(audit_dir, "prep_metadata")
nofilter_results_dir <- file.path(sensitivity_root, "unfiltered_same_pipeline")
nofilter_summaries_dir <- file.path(nofilter_results_dir, "summaries")
safe_dir(audit_dir)
safe_dir(audit_meta_dir)
safe_dir(nofilter_results_dir)
...
run_exact_nofilter <- !identical(tolower(Sys.getenv("RUN_EXACT_NOFILTER_SENSITIVITY", "true")), "false")
...
if (run_exact_nofilter) {
  ...
  nofilter_fit <- lapply(all_model_ids, function(model_id) {
    ...
    if (file.exists(out_file)) {
      show_progress(sprintf("Reutilizando resultado sem filtro ja existente: %s", model_id))
      reused <- summarize_saved_model(out_file, filter_strategy = "unfiltered")$summary
      return(reused[, c("model_id", "model_type", "exposure_main", "n_final", "geocodes", "aic", "bic", "theta")])
    }
...
report_lines <- c(
  "# Review Response - Data Cleaning and Geocode Filter (RS + OFFSET, Exact Sensitivity Without Plots)",
  "",
  ...
  if (run_exact_nofilter) {
    ...
    "- Exact no-filter RS + OFFSET rerun was not executed."
  },
...
writeLines(report_lines, file.path(audit_dir, "REVIEW_RESPONSE_DATA_CLEANING_RS_OFFSET_EXACT.md"))

show_progress("Auditoria exata do filtro de geocodes concluida.")
