...
def main():
    # Diretório com os arquivos indexP
    diretorio_indexP = r"d:\CÓDIGOS\indexP_tempMed_humMed"
    
    # Arquivo de saída
    arquivo_saida = r"d:\CÓDIGOS\indexP_tempMed_humMed_consolidado.csv"
    
    # Arquivo com mapeamento completo
    arquivo_geojson = r"d:\CÓDIGOS\PERNAMBUCO\geojs-26-mun.json"
    
    if not os.path.exists(arquivo_geojson):
        print(f"❌ Arquivo de mapeamento não encontrado: {arquivo_geojson}")
        return
    
    # Carregar mapeamento do GeoJSON
    print("Carregando mapeamento de geocodes do GeoJSON...")
    with open(arquivo_geojson, 'r', encoding='utf-8') as f:
        geojson_data = json.load(f)
    
    # Criar dicionário geocode -> cidade
    mapeamento = {}
    ...
    # Processar cada diretório de cidade
        ...
        if os.path.isdir(caminho_item):
            ...
            if os.path.exists(arquivo_csv):
                try:
                    ...
                    # Adicionar colunas geocode e cidade
                    df['geocode'] = geocode
