...
standardize_population <- function(pop_raw, target_uf = "RJ") {
...
prepare_base_with_population <- function(data_dir, summaries_dir, start_year = 2017, end_year = 2024) {
  show_progress(sprintf("Leitura das bases para Rio de Janeiro (janela %d-%d).", start_year, end_year))
  ...
  path_dengue <- file.path(data_dir, "dengue_rio_de_janeiro_consolidado.csv")
  path_clima <- file.path(data_dir, "climate_rj_2017_2024.csv")
  path_index <- file.path(data_dir, "indexP_rj_2017_2024.csv")
  ...
  pop_std <- standardize_population(pop_raw, target_uf = "RJ")
...
run_multicollinearity_diagnostics <- function(base_data, summaries_dir) {
  show_progress("Diagnostico de multicolinearidade para Rio de Janeiro.")
  ...
  write.csv(cor_pearson, file.path(summaries_dir, "multicolinearidade_cor_pearson_rj_offset.csv"))
  write.csv(cor_spearman, file.path(summaries_dir, "multicolinearidade_cor_spearman_rj_offset.csv"))
  write.csv(vif_all, file.path(summaries_dir, "multicolinearidade_vif_rj_offset.csv"), row.names = FALSE)
  ...
  write.csv(
    vif_combined,
    file.path(summaries_dir, "multicolinearidade_vif_modelos_combinados_rj_offset.csv"),
    row.names = FALSE
  )
...
build_qc_report <- function(summaries_dir, model_results, merge_integrity, lag_summary_all, multicolin) {
  report_file <- file.path(summaries_dir, "RELATORIO_QC_OFFSET_RIO_DE_JANEIRO.md")
