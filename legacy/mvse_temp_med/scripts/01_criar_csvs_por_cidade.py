...
def criar_csvs_por_cidade():
    ...
    # Arquivo de entrada
    arquivo_clima = r"d:\CÓDIGOS\ZENODO\infodengue_sprint_24-25\climate_pe_pos_2016_2.csv"
    
    # Diretório de saída
    dir_saida = r"d:\CÓDIGOS\dados_mvse_cidades_tempMed_humidMed"
    
    # Arquivo com geocodes das cidades
    arquivo_geocodes = r"d:\CÓDIGOS\geocodes_pernambuco.csv"
    ...
    for _, row in df_geocodes.iterrows():
        ...
        try:
            ...
            # Preparar dados no formato MVSE (date, T, H, R)
            df_mvse = pd.DataFrame({
                'date': df_cidade['date'].dt.strftime('%Y-%m-%d'),
                'T': df_cidade['temp_med'],          # Temperatura média
                'H': df_cidade['rel_humid_med'],     # Umidade relativa média
                'R': df_cidade['precip_tot']         # Precipitação total
            })
