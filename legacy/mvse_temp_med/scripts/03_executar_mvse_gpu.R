...
setwd("d:/CÓDIGOS")
dir.create("indexP_tempMed_humMed", showWarnings = FALSE)

# DIRETÓRIO CORRETO - NOSSOS DADOS COM TEMP_MED + REL_HUMID_MED
dir_clima <- "d:/CÓDIGOS/dados_mvse_cidades_tempMed_humMed"
...
processar_cidade_extremo <- function(arquivo_clima) {
  tryCatch({
    ...
    dir_cidade <- file.path("indexP_tempMed_humMed", nome_cidade)
...
saveRDS(relatorio_final, "relatorio_mvse_tempMed_humMed.rds")
