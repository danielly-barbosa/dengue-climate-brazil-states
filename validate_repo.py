...
def check(name, condition, detail="", optional=False):
...
def test_mvse_scripts():
    ...
    states_with_mvse = ["goias", "pernambuco", "rio_de_janeiro", "rio_grande_do_sul"]
...
def test_dlnm_data():
    ...
    for state in states_with_dlnm:
        ...
        climate_alt = list((REPO_ROOT / state / "mvse" / "inputs").glob("*_climate.csv")) if (REPO_ROOT / state / "mvse" / "inputs").exists() else []
        check(f"{state}/dlnm/data/ or mvse/inputs/ has climate_*.csv",
              len(climate_files) > 0 or len(climate_alt) > 0)
...
def test_cross_state():
    ...
    if cs_deriv.exists():
        ...
        check("cross_state/derivative/outputs/ exists (optional; created by export_artifacts.py)",
              outputs_dir.exists(), optional=True)
...
def test_no_geocode_csvs_tracked():
    ...
    geocode_dirs = []
    for state in ["goias", "pernambuco", "rio_de_janeiro", "rio_grande_do_sul"]:
...
ZENODO_DOI = "10.5281/zenodo.22310988"
ZENODO_RECORD_ID = "22310988"
ZENODO_RECORD_URL = f"https://zenodo.org/record/{ZENODO_RECORD_ID}"


def test_zenodo_integration():
    section("19. Zenodo large-file integration")

    readme = (REPO_ROOT / "README.md").read_text(encoding="utf-8")
    check(
        "README.md references Zenodo DOI",
        ZENODO_DOI in readme,
        f"expected substring '{ZENODO_DOI}' in README.md",
    )
    check(
        "README.md references Zenodo record URL",
        ZENODO_RECORD_URL in readme,
        f"expected substring '{ZENODO_RECORD_URL}' in README.md",
    )
    check(
        "README.md does not contain placeholder",
        "ZENODO_PLACEHOLDER" not in readme,
    )
    ...
    if citation_path.exists():
        ...
        check(
            "CITATION.cff references Zenodo DOI",
            ZENODO_DOI in citation,
            f"expected substring '{ZENODO_DOI}' in CITATION.cff",
        )
        check(
            "CITATION.cff does not contain placeholder",
            "ZENODO_PLACEHOLDER" not in citation,
        )

    fetch_script = REPO_ROOT / "scripts" / "fetch_zenodo_data.py"
    check("scripts/fetch_zenodo_data.py exists", fetch_script.exists())
    if fetch_script.exists():
        fetch_src = fetch_script.read_text(encoding="utf-8")
        check(
            "fetch_zenodo_data.py has default record ID",
            ZENODO_RECORD_ID in fetch_src,
            f"expected substring '{ZENODO_RECORD_ID}' in fetch_zenodo_data.py",
        )

    upload_script = REPO_ROOT / "scripts" / "upload_zenodo.py"
    check("scripts/upload_zenodo.py exists", upload_script.exists())
