...
standardize_population <- function(pop_raw, target_uf = "GO") {
  ...
  pop <- pop_raw %>%
    mutate(
      ...
      geocode = as.character(.data[[id_col]]),
...
prepare_base_with_population <- function(data_dir, summaries_dir, start_year = 2017, end_year = 2024) {
  ...
  path_index <- file.path(data_dir, "indexP_goias_consolidado.csv")
  ...
  dengue <- dengue %>% filter(year(date) >= start_year, year(date) <= end_year)
  clima <- clima %>% filter(year(date) >= start_year, year(date) <= end_year)
  indexp <- indexp %>% filter(year(date) >= start_year, year(date) <= end_year)
  ...
  pop_std <- standardize_population(pop_raw, target_uf = "GO")
  pop <- pop_std$population %>% mutate(geocode = as.character(geocode))
  ...
  base <- base %>%
    mutate(
      ...
      # Offset populacional aplicado em escala por 100 mil habitantes.
...
deduplicate_exposure_individual <- function(dados, exposure_var) {
  # Esta deduplicacao vale apenas para o grid de predicao/grafico.
  # Nao remove nem altera linhas do painel de municipios.
...
build_crossbasis_panel <- function(exposure, geocode, lag_max = 12, var_df = 3, lag_df = 3) {
  # Solucao adotada: group=geocode para impedir que lags atravessem fronteiras
  # entre municipios no painel.
  dlnm::crossbasis(
    exposure,
    lag = lag_max,
...
run_individual_model <- function(
  ...
  # Grid deduplicado so para predicao, sem mexer nas linhas do painel.
  deduplicate_exposure_individual(dados, exposure_var)
...
run_combined_model <- function(
  ...
  # Grid deduplicado so para predicao, sem mexer nas linhas do painel.
  deduplicate_exposure_combined(dados, exposure_vars)
...
run_multicollinearity_diagnostics <- function(base_data, summaries_dir) {
  show_progress("Diagnostico de multicolinearidade para Goias.")
  ...
  write.csv(cor_pearson, file.path(summaries_dir, "multicolinearidade_cor_pearson_goias_offset.csv"))
  write.csv(cor_spearman, file.path(summaries_dir, "multicolinearidade_cor_spearman_goias_offset.csv"))
  write.csv(vif_all, file.path(summaries_dir, "multicolinearidade_vif_goias_offset.csv"), row.names = FALSE)
  ...
  write.csv(
    vif_combined,
    file.path(summaries_dir, "multicolinearidade_vif_modelos_combinados_offset.csv"),
    row.names = FALSE
  )
...
build_qc_report <- function(summaries_dir, model_results, merge_integrity, lag_summary_all, multicolin) {
  report_file <- file.path(summaries_dir, "RELATORIO_QC_OFFSET_GOIAS.md")
  ...
  lines <- c(
    ...
    sprintf("- Resultado agregado: %d de %d verificacoes indicaram bloqueio de lag crossing com `group`.", lag_blocked, lag_total),
    "",
...
run_individual_by_name <- function(model_name, data_dir = NULL, results_dir = NULL) {
  ...
  if (!model_name %in% names(specs)) stop(sprintf("Modelo individual invalido: %s", model_name))
...
run_combined_by_name <- function(model_name, data_dir = NULL, results_dir = NULL) {
  ...
  if (!model_name %in% names(specs)) stop(sprintf("Modelo combinado invalido: %s", model_name))
