...
def processar_dados_mvse_tempMed():
    ...
    # Arquivo de entrada
    arquivo_entrada = "d:/CÓDIGOS/ZENODO/infodengue_sprint_24-25/climate_pe_pos_2016_2.csv"
    ...
    try:
        ...
        # Mapeamento de variáveis para MVSE
        mapeamento = {
            'temp_med': 'T',      # Temperatura média (°C)
            'rel_humid_med': 'H', # Umidade relativa média (%)
            'precip_tot': 'R'     # Precipitação total (mm)
        }
        ...
        dir_saida = "d:/CÓDIGOS/dados_mvse_cidades_tempMed_humMed"
        ...
        for geocode in cidades:
            try:
                ...
                if len(dados_cidade) < 50:  # Mínimo de ~1 ano de dados semanais
                    print(f"  Cidade {geocode}: poucos dados ({len(dados_cidade)} registros) - pulando")
                    continue
                
                # Selecionar apenas colunas para MVSE
        ...
        # Verificar alguns arquivos gerados
