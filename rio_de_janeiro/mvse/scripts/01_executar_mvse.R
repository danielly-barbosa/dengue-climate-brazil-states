...
# setwd("d:/CÓDIGOS")
dir.create("rio_de_janeiro/mvse/outputs/indexP_tempMed_humMed", showWarnings = FALSE)

# DIRETÓRIO CORRETO - NOSSOS DADOS COM TEMP_MED + REL_HUMID_MED
dir_clima <- "rio_de_janeiro/mvse/outputs/geocodes"
...
processar_cidade_extremo <- function(arquivo_clima) {
  tryCatch({
    ...
    dir_cidade <- file.path("rio_de_janeiro/mvse/outputs/indexP_tempMed_humMed", nome_cidade)
...
config_inicial <- list(
  ...
  diretorio_resultados = "indexP_tempMed_humMed",
  parametros = list(
    nMCMC = 25000,
    nSample = 120,
    nBurnin = 5000,
...
arquivos_saida <- list.files("indexP_tempMed_humMed", pattern = "\\.estimated_indexP\\.csv$", recursive = TRUE)
...
saveRDS(relatorio_final, "relatorio_mvse_rj.rds")

cat("\nRelatório salvo em: relatorio_mvse_tempMed_humMed.rds\n")
cat("Resultados salvos em: indexP_tempMed_humMed/\n")
